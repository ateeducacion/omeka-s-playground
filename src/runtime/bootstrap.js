...
export const PLAYGROUND_PREPEND_PATH =
  "/persist/mutable/config/playground-prepend.php";
...
// digits, or underscores. Mirrors the blueprint schema's propertyNames pattern.
const PHP_CONSTANT_NAME_PATTERN = /^[A-Z_][A-Z0-9_]*$/;

// Emit guarded `define()` calls for the blueprint-provided PHP constants. The
// engine stays generic: each blueprint (not this engine) decides which
// constants to define before Omeka boots. Booleans map to PHP true/false,
// numbers to numeric literals, and strings to escaped single-quoted literals.
function buildPhpConstantDefines(phpConstants) {
  const defines = [];
  ...
  if (phpConstants && typeof phpConstants === "object") {
    for (const [name, value] of Object.entries(phpConstants)) {
      ...
      if (typeof value === "boolean") {
        phpValue = phpBoolean(value);
      } else if (typeof value === "number") {
        phpValue = String(value);
      } else if (typeof value === "string") {
        phpValue = `'${phpString(value)}'`;
      } else {
        continue;
      }
...
function buildDatabaseIni() {
  return ['driver = "pdo_sqlite"', `path = "${PLAYGROUND_DB_PATH}"`, ""].join(
    "\n",
  );
}
...
`
  ...
  // Blueprint-driven PHP constants. The engine is agnostic about their meaning:
  // whichever blueprint is loaded supplies `phpConstants`, and we define them
  // here (guarded by `!defined(...)`) so they are available before Omeka boots.
  const phpConstantDefines = buildPhpConstantDefines(config.phpConstants);
  const phpConstantsBlock = phpConstantDefines
    ? `\n// PHP constants requested by the active blueprint.\n${phpConstantDefines}\n`
    : "";
...
function extractCsrfField(html) {
  const match = html.match(
    /<input[^>]+type=["']hidden["'][^>]+name=["']([^"']+_csrf|csrf)["'][^>]+value=["']([^"']*)["'][^>]*>/iu,
  );
  if (!match) {
    return null;
  }
...
function isLoginPage(html) {
  return (
    /<body[^>]*class=["'][^"']*\blogin\b[^"']*["']/iu.test(html) ||
...
async function performAutologin(php, config, publish) {
  ...
  const landedOnLogin =
    /\/login(?:\/|$)?/u.test(adminLocation) || isLoginPage(adminHtml);
...
async function resetPersistedState(php) {
  ...
  for (const entry of FS.readdir(persistedRoot)) {
    ...
    removeNodeIfPresent(
      FS,
      `${persistedRoot}/${entry}`.replace(/\/{2,}/gu, "/"),
    );
  }
}
...
function buildBlueprintMediaCachePath(itemIndex, mediaIndex, filename) {
  const safeName = sanitizeMediaFilename(
    filename,
    `media-${itemIndex + 1}-${mediaIndex + 1}.bin`,
  );
...
async function cacheBlueprintMediaFiles({ php, blueprint, publish }) {
  ...
  const items = Array.isArray(stagedBlueprint.items)
    ? stagedBlueprint.items
    : [];
  ...
  for (const [itemIndex, item] of items.entries()) {
    ...
    for (const [mediaIndex, media] of mediaEntries.entries()) {
      ...
      try {
        const directFetch =
          globalThis.__omekaOriginalFetch || globalThis.fetch.bind(globalThis);
        ...
        const cachedName = sanitizeMediaFilename(
          new URL(response.url || sourceUrl, sourceUrl).pathname,
          `media-${itemIndex + 1}-${mediaIndex + 1}.bin`,
        );
        const cachedPath = buildBlueprintMediaCachePath(
          itemIndex,
          mediaIndex,
          cachedName,
        );
        ...
        publish(
          `[warning] Unable to prefetch media "${sourceUrl}" for blueprint item "${item.title || `#${itemIndex + 1}`}": ${detail}`,
          0.56,
        );
      }
    }
...
async function appendPhpIniOverrides(php, config) {
  ...
  await php.writeFile(
    ...
    encoder.encode(buildPhpPrepend(config)),
  );
}
...
export async function bootstrapOmeka({
  ...
  const probeResponse = await php.request(
    new Request("https://playground.internal/playground-probe.php"),
  );
  ...
  await php.writeFile(
    `${OMEKA_ROOT}/playground-install.php`,
    encoder.encode(
      buildInstallScript(
        effectiveConfig,
        manifestState,
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const output = await php.request(
      new Request("https://playground.internal/playground-install.php"),
    );
    ...
    for (const line of outputLines) {
      if (
        line === "omeka-playground-bootstrap-complete" ||
        line === "omeka-playground-bootstrap-continue"
      ) {
        continue;
      }
  ...
  if (!bootstrapComplete) {
    throw new Error(
      "Omeka bootstrap did not complete after repeated module install passes.",
    );
  }

  let readyPath =
    runtimeBlueprint.landingPage || effectiveConfig.landingPath || "/admin";
